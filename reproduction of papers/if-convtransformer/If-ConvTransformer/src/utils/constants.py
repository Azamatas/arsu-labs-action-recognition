...
DATASETS = ["UniMiB"]  # Run UniMiB only, or ['HAPT', 'Opportunity', 'UniMiB'] for all

CLASSIFIERS = ["Deep_Sen_At_TCN_Cr_Br_At_Transformer_torch"]

cal_attitude_angle = True
test_split = 50  # when testing, the testing dataset is separated into 'test_split' pieces
...
def get_HAPT_dataset_param(CUR_DIR, dataset_name):
    ...
    ALL_SUBJECTS_ID = list(range(1, 31))
    WINDOW_SIZE = 128
    OVERLAP = 64
    INPUT_CHANNEL = 9
    POS_NUM = 1
...
def get_UniMiB_dataset_param(CUR_DIR, dataset_name):

    DATA_DIR = r"C:\Users\amanb\PycharmProjects\arsu-labs-action-recognition\datasets\UniMiB-SHAR\data"
    ...
    ActID = list(range(17))
    TRAIN_SUBJECTS_ID = list(range(1, 29))
    TEST_SUBJECTS_ID = [29, 30]
    WINDOW_SIZE = 151
    OVERLAP = 0
    INPUT_CHANNEL = 9  # CRITICAL: must be 9 to match model
    POS_NUM = 1
...
def create_classifier(
    ...
    if classifier_name == "Deep_Sensor_Attn_TCN_Transformer_torch":
        ...
        return (
            Deep_Sensor_Attn_TCN_Transformer_torch.Deep_Sensor_Attn_TCN_Transformer(
                1,
                input_channel,
                64,
                5,
            ...
            Deep_Sensor_Attn_TCN_Transformer_torch,
        )
    ...
    if classifier_name == "Deep_Sen_At_TCN_Cr_Br_At_Transformer_torch":
        ...
        return (
            Deep_Sen_At_TCN_Cr_Br_At_Transformer_torch.Deep_Sen_At_TCN_Cr_Br_At_Transformer(
                1,
                input_channel,
                64,
                5,
            ...
            Deep_Sen_At_TCN_Cr_Br_At_Transformer_torch,
        )
