...
def preprocess_raw_data(
    ...
    for acc_file, gyro_file in zip(acc_files, gyro_files):
        ...
        acc_raw = pd.read_table(
            acc_file, sep=" ", header=None, names=[
                "x", "y", "z"])
        gyro_raw = pd.read_table(
            gyro_file, sep=" ", header=None, names=[
                "x", "y", "z"])

        # per axis(x,y,z) does Z-normalization
        if separate_gravity_flag:
            pre_obj = Preprocess()
            acc_body, acc_grav = pre_obj.separate_gravity(
                acc_raw, cal_attitude_angle)  # seperate gravity from acc
        ...
        for _, _, act_id, act_start, act_end in temp_label_info.values:
            ...
            if separate_gravity_flag:
                ...
                tGravityAccXYZ = preprocess_signal(
                    temp_acc_grav, window_size, overlap)
                tBodyGyroXYZ = preprocess_signal(
                    temp_gyro_raw, window_size, overlap)
                tBodyAccXYZ = preprocess_signal(
                    temp_acc_body, window_size, overlap)
                channel_num = 9
                ...
                column_ind = ["AccX", "AccY", "AccZ",
                              "GyroX", "GyroY", "GyroZ"]
            # the number of windows in current experiment, user and action, put
            # them into features
            # len(tAccXYZ):number of windows, 128:window size, 6:the axises of
            # channels
            features = np.zeros((len(tBodyGyroXYZ), window_size, channel_num))
            # concatenate acc and gyro data into 'feature', then the seg
            # windows of data are put into features
            for i in range(len(tBodyGyroXYZ)):
                if separate_gravity_flag:
                    np_concat = np.concatenate(
                        (tGravityAccXYZ[i], tBodyGyroXYZ[i], tBodyAccXYZ[i]), 1)
                else:
                    np_concat = np.concatenate(
                        (tAccXYZ[i], tBodyGyroXYZ[i]), 1)
...
# def preprocess_raw_data(DATA_DIR, TRAIN_SUBJECTS, ActID, window_size,
# overlap, scaler, separate_gravity_flag=False):
