...
def load_Opportunity_data(DATA_DIR,
                          SUBJECTS,
                          TRIALS,
                          SELEC_LABEL,
                          TRAIN_SUBJECTS_ID,
    ...
    return np.expand_dims(X_train, axis=1), np.expand_dims(X_test, axis=1), y_train.squeeze(
    ), y_test.squeeze(), np.array(User_ids_train), np.array(User_ids_test), label2act, act2label
