...
def scale(signal: pd.DataFrame, scaler="normalize",
          minmax_range: Optional[Tuple[int, int]] = (0, 1)) -> pd.DataFrame:
    if scaler == "normalize":
...
def preprocess_raw_data(DATA_DIR, SUBJECTS, TRIALS, SELEC_LABEL,
    ...
    for sub_id in SUBJECTS:
        ...
        for trail_id in TRIALS:
            ...
            cur_trail = cur_sub_frag[cur_sub_frag[str(
                RUN_ID)] == trail_id].values

            # preprocess the IMU data without shoes
            for pos_id in range(5):
                cur_pos_frag = cur_trail[:, pos_id * 13: (pos_id + 1) * 13]

                cur_pos_acc = pd.DataFrame(
                    cur_pos_frag[:, 0:3], columns=["x", "y", "z"])
                cur_pos_gyro = pd.DataFrame(
                    cur_pos_frag[:, 3:6], columns=["x", "y", "z"])
                ...
                if separate_gravity_flag:
                    ...
                    cur_pos_acc_body, cur_pos_grav = pre_obj.separate_gravity(
                        cur_pos_acc, cal_attitude_angle)  # seperate gravity from acc
                    # Correct the IMU data to NED
                    if to_NED_flag:
                        cur_pos_grav = pre_threeaxis_data(
                            to_NED_flag, cur_pos_grav, cur_pos_quaternion)
                        cur_pos_acc_body = pre_threeaxis_data(
                            to_NED_flag, cur_pos_acc_body, cur_pos_quaternion)
                        cur_pos_gyro = pre_threeaxis_data(
                            to_NED_flag, cur_pos_gyro, cur_pos_quaternion)
                    # norm per sensor
                    cur_pos_grav = scale(cur_pos_grav)
                    cur_pos_acc_body = scale(cur_pos_acc_body)
                    cur_pos_gyro = scale(cur_pos_gyro)

                    cur_pos_data = np.concatenate(
                        (cur_pos_grav, cur_pos_gyro, cur_pos_acc_body), axis=1)

                elif separate_gravity_flag == False:
                    if to_NED_flag:
                        cur_pos_acc = pre_threeaxis_data(
                            to_NED_flag, cur_pos_acc, cur_pos_quaternion)
                        cur_pos_gyro = pre_threeaxis_data(
                            to_NED_flag, cur_pos_gyro, cur_pos_quaternion)
                    # norm per sensor
                    ...
                    cur_pos_data = np.concatenate(
                        (cur_pos_acc, cur_pos_gyro), axis=1)

                if pos_id == 0:
                    cur_trail_frag = cur_pos_data
                else:
                    cur_trail_frag = np.concatenate(
                        (cur_trail_frag, cur_pos_data), axis=1)
            # preprocess the IMU data of shoes
            for shoe_id in range(2):
                cur_shoe_frag = cur_trail[:, (5 *
                                              13 +
                                              shoe_id *
                                              16): (5 *
                                                    13 +
                ...
                cur_shoe_acc = pd.DataFrame(
                    cur_shoe_frag[:, 6:9], columns=["x", "y", "z"])
                cur_shoe_gyro = pd.DataFrame(
                    cur_shoe_frag[:, 9:12], columns=["x", "y", "z"])
                cur_shoe_quaternion = cur_shoe_frag[:, 0:3] * 2 * math.pi / 360

                if separate_gravity_flag:
                    # separate gravity and linear acc
                    pre_obj = Preprocess(30)
                    cur_shoe_acc_body, cur_shoe_grav = pre_obj.separate_gravity(
                        cur_shoe_acc, cal_attitude_angle)  # seperate gravity from acc
                    if to_NED_flag:
                        cur_shoe_grav, cur_shoe_acc_body, cur_shoe_gyro = correct_orientation9(
                            cur_shoe_grav, cur_shoe_acc_body, cur_shoe_gyro, cur_shoe_quaternion)
                    # norm per sensor
                    cur_shoe_grav = scale(cur_shoe_grav)
                    cur_shoe_acc_body = scale(cur_shoe_acc_body)
                    cur_shoe_gyro = scale(cur_shoe_gyro)

                    cur_shoe_data = np.concatenate(
                        (cur_shoe_grav, cur_shoe_gyro, cur_shoe_acc_body), axis=1)

                if not separate_gravity_flag:
                    if to_NED_flag:
                        cur_shoe_acc, cur_shoe_gyro = correct_orientation6(
                            cur_shoe_acc, cur_shoe_gyro, cur_shoe_quaternion)
                    # norm per sensor
                    cur_shoe_acc = scale(cur_shoe_acc)
                    cur_shoe_gyro = scale(cur_shoe_gyro)

                    cur_shoe_data = np.concatenate(
                        (cur_shoe_acc, cur_shoe_gyro), axis=1)

                cur_trail_frag = np.concatenate(
                    (cur_trail_frag, cur_shoe_data), axis=1)
            # append label/subject/trial IDs
            cur_trail_frag = np.concatenate(
                (cur_trail_frag, cur_trail[:, SELEC_LABEL:(SELEC_LABEL + 1)]), axis=1)
            cur_trail_frag = np.concatenate(
                (cur_trail_frag, cur_trail[:, SUBJECT_ID:(SUBJECT_ID + 1)]), axis=1)
            cur_trail_frag = np.concatenate(
                (cur_trail_frag, cur_trail[:, RUN_ID:(RUN_ID + 1)]), axis=1)

            for (label_id, label) in enumerate(LABELS):

                cur_label_frag = pd.DataFrame(
                    cur_trail_frag[cur_trail_frag[:, -3] == label])
                ...
                cur_label_frag = pd.DataFrame(
                    cur_label_frag.values[:, 0:(cur_label_frag.shape[1] - 3)])

                cur_label_segments_mid = preprocess_signal(
                    cur_label_frag, window_size, overlap)
                ...
                cur_label_trial_labels = [
                    trail_id] * len(cur_label_segments_mid)
                ...
                # distinguish whether the current 'features' should be put into
                # train set or test set, according to the predivided user info
                if (sub_id in TRAIN_SUBJECTS_ID) and (
                        trail_id in TRAIN_SUBJECTS_TRIAL_ID):
                    if len(X_train) == 0:
