...
def scale(
    signal: pd.DataFrame,
    scaler="normalize",
    minmax_range: Optional[Tuple[int, int]] = (0, 1),
) -> pd.DataFrame:
    if scaler == "normalize":
    ...
    for sub_id in SUBJECTS:
        ...
        for trail_id in TRIALS:
            ...
            for pos_id in range(5):
                ...
                if separate_gravity_flag:
                    ...
                    cur_pos_acc_body, cur_pos_grav = pre_obj.separate_gravity(
                        cur_pos_acc, cal_attitude_angle
                    )  # seperate gravity from acc
                ...
                elif not separate_gravity_flag:
                    if to_NED_flag:
            ...
            for shoe_id in range(2):
                ...
                if separate_gravity_flag:
                    ...
                    cur_shoe_acc_body, cur_shoe_grav = pre_obj.separate_gravity(
                        cur_shoe_acc, cal_attitude_angle
                    )  # seperate gravity from acc
                    if to_NED_flag:
                    ...
                    cur_shoe_data = np.concatenate(
                        (cur_shoe_grav, cur_shoe_gyro, cur_shoe_acc_body),
                        axis=1,
                    )
            ...
            cur_trail_frag = np.concatenate(
                (
                    cur_trail_frag,
                    cur_trail[:, SELEC_LABEL : (SELEC_LABEL + 1)],
                ),
                axis=1,
            )
            cur_trail_frag = np.concatenate(
                (cur_trail_frag, cur_trail[:, SUBJECT_ID : (SUBJECT_ID + 1)]),
                axis=1,
            )
