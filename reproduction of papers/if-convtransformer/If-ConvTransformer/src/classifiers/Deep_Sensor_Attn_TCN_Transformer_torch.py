...
#        out_attitude = sum_attitude
        ...
        # (batch_size, time_length, scale_num, 3(xyz)*feature_chnnl)
        out_dynamic = out_dynamic.reshape(
            out_dynamic.shape[0], out_dynamic.shape[1], out_dynamic.shape[2], -1)
        # time-step-wise sensor attention, scale_attn:(batch_size, time_length,
        # scale_num, 1)
        ...
        # (batch_size, time_length, scale_num, 3(xyz), feature_chnnl)
        out_dynamic = out_dynamic.reshape(
            out_dynamic.shape[0], out_dynamic.shape[1], out_dynamic.shape[2], 3, -1)
        # (batch_size, time_length, scale_num*3(xyz), feature_chnnl)
        out_dynamic = out_dynamic.reshape(
            out_dynamic.shape[0], out_dynamic.shape[1], out_dynamic.shape[2] * 3, -1)
...
#            kernel_size_acc = padding_grav*2+1
    ...
    def forward(self, x):
        ...
        for i in range(self.scale_num):
            ...
            attention_attitude = F.softmax(
                attention_attitude, dim=2).unsqueeze(-1)
            ...
            if i == 0:
                ...
                out_attitude = torch.cat(
                    [out_attitude, (out_grav + out_mag + out_gyro)], dim=2)
...
class Deep_Sensor_Attn_TCN_Transformer(nn.Module):
    def __init__(self, input_2Dfeature_channel, input_channel, feature_channel,
        ...
        self.IMU_fusion_block = IMU_Fusion_Block(
        ...
        self.transition = nn.Sequential(
            nn.Conv1d(feature_channel * (input_channel - reduced_channel)
                      * scale_num, feature_channel_out, 1, 1),
...
def train_op(network, EPOCH, BATCH_SIZE, LR,
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
        optimizer, 'min', factor=0.5, patience=5, min_lr=LR / 10, verbose=True)
    ...
    for epoch in range(EPOCH):
        ...
        loss_test, accuracy_test, macro_f1_test = get_test_loss_acc(
            network, loss_function, X_test, y_test, test_split)
        test_flag = False
        network.train()
