...
class IMU_Fusion_Block(nn.Module):
    def __init__(
        ...
        self.attention = nn.Sequential(nn.Linear(3 * feature_channel, 1), nn.Tanh())

        self.attention_scale = nn.Sequential(nn.Linear(3 * feature_channel, 1), nn.Tanh())
...
class Deep_Sensor_Attn_TCN_Transformer(nn.Module):
    def __init__(
        ...
        self.transition = nn.Sequential(
            nn.Conv1d(
                feature_channel * (input_channel - reduced_channel) * scale_num,
                feature_channel_out,
                1,
                1,
            ),
