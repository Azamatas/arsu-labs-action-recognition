...
CUR_DIR = os.path.dirname(os.path.abspath(
    __file__))  # Path to current directory
...
def create_cuda_classifier(
    ...
    classifier, classifier_func = create_classifier(
        dataset_name, classifier_name, INPUT_CHANNEL, POS_NUM, data_length, nb_classes)
...
for dataset_name in DATASETS:
    ...
    for classifier_name in CLASSIFIERS:
        ...
        EXEC_TIME, LOG_DIR, MODEL_DIR, logger = logging_settings(
            classifier_name, CUR_DIR, dataset_name)
        time2 = 0
        ...
        for subject_id in ALL_SUBJECTS_ID:
            ...
            TRAIN_SUBJECTS_ID = list(
                set(ALL_SUBJECTS_ID).difference(
                    set(TEST_SUBJECTS_ID)))
            ...
            X_train, y_train = get_loso_train_test_data(
                TRAIN_SUBJECTS_ID, All_users, All_data, All_labels)
            X_test, y_test = get_loso_train_test_data(
                TEST_SUBJECTS_ID, All_users, All_data, All_labels)

            if classifier_name in ['Comple_Filter_Conv_Transformer_torch']:
                for (pos_id, pos) in enumerate(range(POS_NUM)):
                    if pos_id == 0:
                        X_train_cat = complementary_filter(X_train[:, :, (X_train.shape[2] // POS_NUM) * pos_id:(
                            X_train.shape[2] // POS_NUM) * (pos_id + 1), :], COMPLEMAENTARY_ALPHA)
                        X_test_cat = complementary_filter(X_test[:, :, (X_train.shape[2] // POS_NUM) * pos_id:(
                            X_train.shape[2] // POS_NUM) * (pos_id + 1), :], COMPLEMAENTARY_ALPHA)
                    else:
                        X_train_mid = complementary_filter(X_train[:, :, (X_train.shape[2] // POS_NUM) * pos_id:(
                            X_train.shape[2] // POS_NUM) * (pos_id + 1), :], COMPLEMAENTARY_ALPHA)
                        X_test_mid = complementary_filter(X_test[:, :, (X_train.shape[2] // POS_NUM) * pos_id:(
                            X_train.shape[2] // POS_NUM) * (pos_id + 1), :], COMPLEMAENTARY_ALPHA)
                        X_train_cat = np.concatenate(
                            (X_train_cat, X_train_mid), axis=2)
                        X_test_cat = np.concatenate(
                            (X_test_cat, X_test_mid), axis=2)
                X_train = X_train_cat
                X_test = X_test_cat
            ...
            if classifier_name in [
                ...
                for (pos_id, pos) in enumerate(range(POS_NUM)):
                    if pos_id == 0:
                        X_train_cat = STFT_transform(X_train[:, :, (X_train.shape[2] // POS_NUM) * pos_id:(
                            X_train.shape[2] // POS_NUM) * (pos_id + 1), :], y_train, STFT_intervals, test_split)
                        X_test_cat = STFT_transform(X_test[:, :, (X_train.shape[2] // POS_NUM) * pos_id:(
                            X_train.shape[2] // POS_NUM) * (pos_id + 1), :], y_test, STFT_intervals, test_split)
                    else:
                        X_train_mid = STFT_transform(X_train[:, :, (X_train.shape[2] // POS_NUM) * pos_id:(
                            X_train.shape[2] // POS_NUM) * (pos_id + 1), :], y_train, STFT_intervals, test_split)
                        X_test_mid = STFT_transform(X_test[:, :, (X_train.shape[2] // POS_NUM) * pos_id:(
                            X_train.shape[2] // POS_NUM) * (pos_id + 1), :], y_test, STFT_intervals, test_split)
                        X_train_cat = np.concatenate(
                            (X_train_cat, X_train_mid), axis=1)
                        X_test_cat = np.concatenate(
                            (X_test_cat, X_test_mid), axis=1)
                X_train = X_train_cat
                X_test = X_test_cat
            ...
            log_HyperParameters(logger, BATCH_SIZE, EPOCH,
                                LR, len(ALL_SUBJECTS_ID))
            ###################################################################

            ##### SPLIT TRAINSET TO TRAIN AND VAL DATASETS #####
            # Split data by preserving the percentage of samples for each class.
            # here shuffle=False due to trainset has been shuffled through
            # shuffle_trainset func
            if subject_id == min(ALL_SUBJECTS_ID):
                models, scores, log_training_duration = initialize_saving_variables(
                    X_train, X_test, nb_classes, len(ALL_SUBJECTS_ID))
            ...
            ####################################################
            aa = X_val.squeeze()
            # create classifier, put it into cuda and get the parameter number
            # of it
            classifier, classifier_func, classifier_parameter = create_cuda_classifier(
                dataset_name, classifier_name, INPUT_CHANNEL, POS_NUM, X_train.shape[-1], nb_classes)

            ###### TRAIN FOR EACH FOLD, if trained, print 'Already_done' ######
            # log train, validation and test dataset info, log the network
            log_redivdataset_network_info(
                logger,
                subject_id,
                X_tr,
                X_val,
            ...
            output_directory_models = MODEL_DIR + \
                '\\SUBJECT_' + str(subject_id) + '\\'
            flag_output_directory_models = create_directory(
                output_directory_models)
            if PATTERN == 'TRAIN':
                flag_output_directory_models = PATTERN
            if flag_output_directory_models is not None:
                # for each subject, train the network and save the best
                # validation model
                print('SUBJECT_' + str(subject_id) + ': start to train')
                history, per_training_duration, log_training_duration = classifier_func.train_op(
                    classifier, EPOCH, BATCH_SIZE, LR, X_tr, Y_tr, X_val, Y_val, X_test, y_test, output_directory_models, log_training_duration, test_split)
            else:
                print('Already_done: ' + 'SUBJECT_' + str(subject_id))
                # read the training duration of current subject
                per_training_duration = pd.read_csv(
                    output_directory_models + 'score.csv', skiprows=1, nrows=1, header=None)[1][0]
            ...
            pred_train, pred_valid, pred_test, scores, time_duration = classifier_func.predict_tr_val_test(classifier, nb_classes, ACT_LABELS,
                                                                                                           ...
                                                                                                           # scores
                                                                                                           # =
                                                                                                           # classifier_func.predict_tr_val_test(classifier,
                                                                                                           # nb_classes,
                                                                                                           # ACT_LABELS,
                                                                                                           X_tr, X_val, X_test,
                                                                                                           Y_tr, Y_val, y_test,
        ...
        # Log the inference time including data loading, preprocessing and
        # model inference
        preprocess_time, inference_time = log_inference_time(
            start1, end1, y_train, y_test, time2, dataset_name, classifier_name, len(ALL_SUBJECTS_ID), logger)

        save_classifiers_comparison(
            MODELS_COMP_LOG_DIR,
            CLASSIFIERS,
            classifier_name,
            scores,
