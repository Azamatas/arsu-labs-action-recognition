...
from utils.load_Opportunity_dataset.load_Opportunity_dataset import (
    load_Opportunity_data,
)
from utils.load_UniMiB_dataset.load_UniMiB_dataset import load_UniMiB_data
...
def load_raw_data(dataset_name):
    ...
    if dataset_name == "HAPT":
        ...
        ) = get_HAPT_dataset_param(CUR_DIR, dataset_name)
        ...
        # ADDED: Use get_UniMiB_dataset_param from constants.py
        (
            DATA_DIR,
            MODELS_COMP_LOG_DIR,
            ACT_LABELS,
            ...
            POS_NUM,
            separate_gravity_flag,
        ) = get_UniMiB_dataset_param(CUR_DIR, dataset_name)

        # ADDED: Load UniMiB data
        X, y, subjects = load_UniMiB_data(DATA_DIR)

        # ADDED: Build label mappings
        label2act = {i: ACT_LABELS[i] for i in range(len(ACT_LABELS))}
        act2label = {ACT_LABELS[i]: i for i in range(len(ACT_LABELS))}

        # ADDED: Initial LOSO split (will be overridden in LOSO loop)
        ALL_SUBJECTS_ID = np.unique(subjects).tolist()
        train_idx = np.isin(subjects, TRAIN_SUBJECTS_ID)
        test_idx = np.isin(subjects, TEST_SUBJECTS_ID)

        X_train, X_test = X[train_idx], X[test_idx]
        y_train, y_test = y[train_idx], y[test_idx]
        User_ids_train = subjects[train_idx]
        User_ids_test = subjects[test_idx]
        ...
        (
            ...
            separate_gravity_flag,
            to_NED_flag,
        ) = get_Opportunity_dataset_param(CUR_DIR, dataset_name)
        ...
        for subject_id in ALL_SUBJECTS_ID:
            ...
            if classifier_name in ["Comple_Filter_Conv_Transformer_torch"]:
                for pos_id, pos in enumerate(range(POS_NUM)):
                    if pos_id == 0:
                        X_train_cat = complementary_filter(
                            X_train[
                                :,
                                :,
                                (X_train.shape[2] // POS_NUM) * pos_id : (X_train.shape[2] // POS_NUM) * (pos_id + 1),
                                :,
                            ],
                            COMPLEMAENTARY_ALPHA,
                        )
                        X_test_cat = complementary_filter(
                            X_test[
                                :,
                                :,
                                (X_train.shape[2] // POS_NUM) * pos_id : (X_train.shape[2] // POS_NUM) * (pos_id + 1),
                        ...
                        X_train_mid = complementary_filter(
                            X_train[
                                :,
                                :,
                                (X_train.shape[2] // POS_NUM) * pos_id : (X_train.shape[2] // POS_NUM) * (pos_id + 1),
                        ...
                        X_test_mid = complementary_filter(
                            X_test[
                                :,
                                :,
                                (X_train.shape[2] // POS_NUM) * pos_id : (X_train.shape[2] // POS_NUM) * (pos_id + 1),
            ...
            pred_train, pred_valid, pred_test, scores, time_duration = classifier_func.predict_tr_val_test(
                classifier,
                nb_classes,
                ACT_LABELS,
                X_tr,
